...
class TrainDatasetCombination(Dataset):
    def __init__(self, h5_file, patch_size, scale):
        super(TrainDatasetCombination, self).__init__()
        self.h5_file = h5_file
        self.patch_size = patch_size
        self.scale = scale
    ...
    def __getitem__(self, idx):
        with h5py.File(self.h5_file, 'r') as f:
            rdn_pred = f['rdn'][str(idx)][::]
            gmu = f['gmu'][str(idx)][::]
            hr = f['hr'][str(idx)][::]
            lr = f['lr'][str(idx)][::]
            
            
            rdn_pred = rdn_pred.astype(np.float32).transpose([2, 0, 1]) 
            gmu = gmu.astype(np.float32).transpose([2, 0, 1]) 
            hr = hr.astype(np.float32).transpose([2, 0, 1]) 
            lr = lr.astype(np.float32).transpose([2, 0, 1]) 
            
            return rdn_pred, gmu, hr, lr
...
class EvalDatasetCombination(Dataset):
    def __init__(self, h5_file):
        super(EvalDatasetCombination, self).__init__()
        self.h5_file = h5_file

    def __getitem__(self, idx):
        with h5py.File(self.h5_file, 'r') as f:
            rdn_pred = f['rdn'][str(idx)][::].astype(np.float32).transpose([2, 0, 1]) 
            gmu = f['gmu'][str(idx)][::].astype(np.float32).transpose([2, 0, 1])
            hr = f['hr'][str(idx)][::].astype(np.float32).transpose([2, 0, 1]) 
            lr = f['lr'][str(idx)][::].astype(np.float32).transpose([2, 0, 1]) 
