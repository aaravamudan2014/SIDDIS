...
class ESRT(nn.Module):
    def __init__(self, topo_inclusion, num_features, dropout_prob_input, dropout_prob_topo_1, 
                 dropout_prob_topo_2, upscale=4, conv=common.default_conv):
        ...
        if topo_inclusion == "beggining":
            self.output_2 = nn.Conv2d(1+num_features*2, num_channels, kernel_size=3, padding=3 // 2)
        elif topo_inclusion in ["vertical", "horizontal"] :
            self.output_2 = nn.Conv2d(1+num_features, num_channels, kernel_size=3, padding=3 // 2)
        elif topo_inclusion == "none":
            self.output_2 = nn.Conv2d(1, num_channels, kernel_size=3, padding=3 // 2)
    ...
    def forward(self, x, test=False, topo_1=None, topo_2=None):
        ...
        if self.topo_inclusion in ["beggining", "horizontal"]:
            ...
            x_topo_1 = torch.cat([sfe2_topo_1], 1)
        ...
        if self.topo_inclusion in ["beggining", "vertical"]:
            ...
            x_topo_2 = torch.cat([sfe2_topo_2], 1)
